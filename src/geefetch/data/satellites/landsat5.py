...
class Landsat5Base(SatelliteABC):
    _bands = [
        "SR_B1",
        "SR_B2",
        "SR_B3",
    ...
    def get_col(
        ...
        def maskLandsat5cloud(im: ee.Image) -> ee.Image:
        ...
        return landsat_col.map(maskLandsat5cloud)
        # return landsat_col.map(maskLandsat8cloud).map(applyBRDF_L8)


class Landsat5GEE(Landsat5Base):
    def get(
        self,
        aoi: BoundingBox,
        start_date: str,
        ...
        **kwargs: Any,
        ...
        landsat_im : DownloadableGEEImage
            A Landsat 5 composite image of the specified AOI and time range.
        """
    ...
    def full_name(self) -> str:
        return "Landsat 5 (GEE)"


class Landsat5(Landsat5Base):
    def get_time_series(
        self,
        aoi: BoundingBox,
        start_date: str,
        ...
        **kwargs: Any,
        ...
        landsat_im: DownloadableGeedimImageCollection
            A Landsat 5 time series collection of the specified AOI and time range.
        """
        for kwarg in kwargs:
        ...
        if n_images == 0:
            log.error(
                f"Found 0 Landsat 5 image." f"Check region {aoi.transform(WGS84)}."
            )
            raise RuntimeError("Collection of 0 Landsat 5 image.")
        for feature in info["features"]:
            ...
            if Polygon(
                ...
                images[id_.removeprefix("LANDSAT/LT05/C02/T1_L2/")] = PatchedBaseImage(
                    im
                )
    ...
    def get(
        ...
        **kwargs: Any,
        ...
        if n_images > 500:
            log.warn(
                f"Landsat 5 mosaicking with a large amount of images (n={n_images}). Expect slower download time."
            )
            log.info("Change cloud masking parameters to lower the amount of images.")
        if n_images == 0:
            log.error(
                f"Found 0 Landsat 5 image for given parameters. Check region {aoi.transform(WGS84)}"
            )
        log.debug(f"Landsat 5 mosaicking with {n_images} images.")
